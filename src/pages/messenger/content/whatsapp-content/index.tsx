...
const WhatsappContent = ({
  ...
  const {
    data,
    isPending,
    refetch: whatsappMessagesRefetch,
  } = useQuery({
    queryKey: ['getWhatsAppMessages', chatID],
  ...
  const hasAiAssistAgent = (Array.isArray(aiSettings) ? aiSettings : []).some(
    (setting: any) =>
      setting?.name === AI_SETTINGS_TYPES.WHATSAPP &&
  ...
  const hasRecentInboundMessage = useMemo(() => {
    const twentyFourHoursAgo = Date.now() - 24 * 60 * 60 * 1000;

    return (Array.isArray(data) ? data : []).some((item: any) => {
      const direction = `${item?.direction || ''}`.toLowerCase();
      if (direction !== `${ACTIVITYLIST.Inbound}`.toLowerCase()) return false;

      const messageTime = new Date(item?.updatedAt || item?.createdAt || 0).getTime();
  ...
  const { mutate: mutateSendMessage, isPending: isPendingWhatsAppMessage } = useMutation({
  ...
  const handleMessageSubmit = () => {
    if (isPendingWhatsAppMessage || !message.trim() || !hasRecentInboundMessage) {
      return;
    }
              ...
              <textarea
                ...
                placeholder={
                  hasRecentInboundMessage
                    ? 'Write a message...'
                    : 'Outbound messages are enabled only after an inbound message in the last 24 hours'
                }
                onKeyDown={(e) => {
              ...
              <div className="cursor-pointer flex items-center justify-center rounded-full w-[40px] min-w-[40px] h-[40px]  bg-primary text-white hover:bg-[#FBE2C8]/40 hover:text-[#2E2D35]/80">
                {isPendingWhatsAppMessage ? (
                  ...
                  <button
                    ...
                    disabled={isPendingWhatsAppMessage || !hasRecentInboundMessage}
                    className="cursor-pointer"
                  >
