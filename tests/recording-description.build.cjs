...
// src/lib/recording-description.ts
var recording_description_exports = {};
__export(recording_description_exports, {
  describeRecording: () => describeRecording
});
module.exports = __toCommonJS(recording_description_exports);
var DIRECTION_SENTENCE = {
...
var UNKNOWN_DIRECTION = "Calls are recorded automatically.";
var describeRecording = (state) => {
  const parts = [];
  if (state.automaticEnabled) {
    const direction = String(state.direction ?? "").trim().toLowerCase();
    parts.push(
      DIRECTION_SENTENCE[direction] ?? UNKNOWN_DIRECTION
    );
  }
