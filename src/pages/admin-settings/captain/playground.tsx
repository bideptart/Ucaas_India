  ...
  return (
    <div className="flex h-full w-full flex-col gap-5 p-6">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-lg font-bold text-[#2E2D35] dark:text-mcm-ink">Playground</div>
          <div className="text-sm text-[#9A948F] dark:text-mcm-ink-3">Test your assistant live before deploying it.</div>
        </div>
        <select
          value={assistantId}
          onChange={(e) => {
      ...
      <div
        ref={scrollRef}
        className="flex-1 space-y-4 overflow-y-auto rounded-2xl border border-[#EEE7DD] dark:border-mcm-line bg-[#FBE2C8]/50 dark:bg-mcm-surface-3 p-5"
      >
        {messages.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center gap-2 text-sm text-[#9A948F] dark:text-mcm-ink-3">
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex items-end gap-2 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {m.sources.map((s) => (
                    <span
                      ...
                      className="inline-flex items-center gap-1 rounded-full border border-[#EEE7DD] dark:border-mcm-line bg-white dark:bg-mcm-surface px-2.5 py-1 text-xs text-[#9A948F] dark:text-mcm-ink-3"
                    >
            ...
            {m.role === 'user' && (
              <div className="flex size-7 shrink-0 items-center justify-center rounded-full bg-[#F0DFC5] dark:bg-mcm-surface-3 text-[#9A948F] dark:text-mcm-ink-3">
        ...
        {isSending && (
          <div className="flex items-end gap-2">
            ...
            <div className="rounded-2xl rounded-bl-sm border border-gray-100 bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface backdrop-blur-[12px] px-3.5 py-2.5 text-sm text-[#9A948F] dark:text-mcm-ink-3 shadow-sm">
              Typing...
            </div>
          </div>
        )}
