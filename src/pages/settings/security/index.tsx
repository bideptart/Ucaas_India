...
const Security = () => {
  ...
  return (
    <section className="w-full flex flex-col overflow-x-auto overflow-y-hidden">
      <div className="flex items-center justify-between p-3 border-b border-[rgba(225,200,165,0.9)] dark:border-mcm-line min-h-[65px] bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface-3 backdrop-blur-[12px]">
        <div>
          <p className="text-[#2E2D35] dark:text-mcm-ink font-semibold text-lg">Security & Privacy</p>
          <p className="text-[#9A948F] dark:text-mcm-ink-3 text-xs">
            Your password, and every device currently signed in as you.
          </p>
        </div>
      </div>
      <div className="gap-3 flex flex-col w-full h-full p-3">
        <div className="flex sm:flex-row flex-col sm:items-center justify-between gap-4 bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface-3 backdrop-blur-[12px] p-4 rounded-lg border border-[rgba(225,200,165,0.9)] dark:border-mcm-line">
          <div className="flex flex-col gap-1 sm:w-1/2 w-full">
            <p className="flex items-center gap-2 text-[#2E2D35] dark:text-mcm-ink font-semibold text-sm">
        ...
        <div className="flex sm:flex-row flex-col items-center justify-between gap-4 bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface-3 backdrop-blur-[12px] p-4 rounded-lg border border-[rgba(225,200,165,0.9)] dark:border-mcm-line">
              <div className="flex flex-col gap-1 sm:w-1/2 w-full">
                <p className="text-[#2E2D35] dark:text-mcm-ink font-semibold text-sm">Sign out everywhere</p>
                <p className="text-[#9A948F] dark:text-mcm-ink-3 text-xs">
        ...
        <div className="w-full flex sm:flex-row flex-col items-center justify-between gap-5">
          <p className="text-[#2E2D35] dark:text-mcm-ink text-sm">
        ...
        <div className="gap-3 flex flex-col w-full md:h-[calc(100vh-18.5rem)]  overflow-y-auto pr-1">
          {isLoading ? (
            ...
            ownDevices?.map((item: any) => {
              return (
                <div
                  className="border cursor-pointer p-3 flex sm:flex-row flex-col  gap-2 rounded-lg sm:justify-between bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface-3 backdrop-blur-[12px]
                  "
                >
                  <div className="flex sm:items-center xs:justify-start xs:items-start gap-3 w-full">
                    ...
                    <div className="flex flex-col w-full gap-1">
                      <div className="flex flex-col items-start">
                        <p className="text-[#2E2D35] dark:text-mcm-ink font-medium text-sm">
                          {capitalizeFirstLetter(
                            `${item?.user_detail?.first_name || ''} ${item?.user_detail?.last_name || ''}`.trim(),
                          ) || 'Unknown User'}
                        </p>
                        <p className="text-[#9A948F] dark:text-mcm-ink-3 text-xs">{item?.user_detail?.email || ''}</p>
                      </div>
                      <div className="flex flex-col gap-1">
                        <p className="text-[#9A948F] dark:text-mcm-ink-3 text-xs">User Agent: {item?.user_agent}</p>
                        <p className="text-[#9A948F] dark:text-mcm-ink-3 text-xs">IP Address: {item?.ip_address}</p>
                      </div>
                    </div>
                  </div>
