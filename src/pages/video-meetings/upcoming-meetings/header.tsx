...
const MeetingHeader = ({ formInstance, showActions = true }: any) => {
  ...
  return (
    <div className="mx-auto max-w-250 flex w-full flex-col gap-6 sm:pt-3">
      <div
        className="relative w-full flex flex-col gap-7 overflow-hidden rounded-[28px] border border-white/60 bg-white/45 dark:border-[rgba(100,116,139,0.25)] dark:bg-[rgba(30,41,59,0.5)] backdrop-blur-2xl
      sm:p-9 p-5
      ...
      >
        <div
          aria-hidden
          className="vm-sunset-glow pointer-events-none absolute -right-20 -top-24 h-64 w-64 rounded-full blur-[110px]"
          style={{ background: 'rgba(231,139,80,0.16)' }}
        />
        <div
          aria-hidden
          className="vm-sunset-glow pointer-events-none absolute -left-14 -bottom-20 h-48 w-48 rounded-full blur-[95px]"
        ...
        <div className="relative z-10 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4">
            <div
              aria-hidden
              className="vm-sunset-grad hidden sm:flex mt-5 h-14 w-14 shrink-0 items-center justify-center rounded-2xl text-white shadow-[0_6px_18px_rgba(231,139,80,0.35)]"
            ...
            <div className="flex flex-col gap-2 w-full max-w-[420px]">
              <span
                className="vm-sunset-ink text-[10px] font-bold uppercase tracking-[0.14em]"
                style={{ color: '#B5642F' }}
              >
                Video Conferencing
              </span>
              <div
                className="vm-sunset-title w-full text-2xl sm:text-[28px] leading-tight font-extrabold"
                style={{ color: '#8A3F1C' }}
              >
                Video Meetings
              </div>
          ...
          {showActions && (
            <div className="flex w-full flex-col gap-2.5 sm:flex-row sm:items-center lg:w-auto">
              {videAccess?.create && (
                <div
                  className="vm-sunset-grad flex items-center justify-center gap-2 min-h-11 px-5 w-full sm:w-auto cursor-pointer rounded-xl text-white shadow-[0_6px_18px_rgba(231,139,80,0.4)] transition-transform hover:-translate-y-0.5 "
                  style={{ background: 'linear-gradient(135deg, #E78B50, #D9652E)' }}
                  onClick={() => {
              ...
              <div className="vm-sunset-segctl flex items-center gap-1 rounded-xl border border-white/80 bg-white/60 backdrop-blur-md p-1 shadow-[0_2px_10px_rgba(120,60,20,0.06),inset_0_1px_0_rgba(255,255,255,0.9)] w-full sm:w-auto">
                <div
                  onClick={() => setModalState(true)}
                  className="vm-sunset-segctl-item flex flex-1 sm:flex-none items-center justify-center gap-2 min-h-9 px-4 cursor-pointer rounded-lg text-gray-800 transition-colors hover:bg-white/90"
                >
                ...
                {videAccess?.create && (
                  <>
                    <span className="vm-sunset-segctl-divider h-5 w-px bg-gray-900/10" aria-hidden />
                    <div
                      className="vm-sunset-segctl-item flex flex-1 sm:flex-none items-center justify-center gap-2 min-h-9 px-4 cursor-pointer rounded-lg text-gray-800 transition-colors hover:bg-white/90"
