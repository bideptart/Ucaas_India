...
const FacebookContent = ({
  ...
  const {
    data,
    isPending,
    refetch: facebookMessagesRefetch,
  } = useQuery({
    queryKey: ['getFacebookMessages', chatID],
  ...
  const hasAiAssistAgent = (Array.isArray(aiSettings) ? aiSettings : []).some(
    (setting: any) =>
      setting?.name === AI_SETTINGS_TYPES.FACEBOOK &&
  ...
  const { mutate: mutateSendMessage, isPending: isPendingFacebookMessage } = useMutation({
    mutationFn: sendWhatsAppMessage,
    mutationKey: ['SendFacebookMessages'],
    onSuccess: () => {
  ...
  const handleMessageSubmit = () => {
    if (isPendingFacebookMessage || !message.trim()) {
      return;
    }
        ...
        <div className="w-full min-h-0 flex-1 flex flex-col gap-3 lg:min-w-0">
          <div className="flex flex-1 min-h-0 flex-col gap-2 overflow-auto p-3" ref={scrollRef}>
            {isPending ? (
              ...
              groupedMessage.map((item: any, itemIndex: number) => (
                <React.Fragment key={item.date || itemIndex}>
                  ...
                  {item?.message && item?.message.length
                    ? item?.message.map((message: any, msgIndex: number) => {
                        ...
                        return (
                          <div
                            key={message?.id || message?._id || msgIndex}
          ...
          <div className="flex flex-col relative justify-between w-full gap-2.5 p-3 bg-white">
            <div className="relative w-full h-full flex gap-2 sm:gap-3 justify-between items-center">
              ...
              <div className="cursor-pointer flex items-center justify-center rounded-full w-[40px] min-w-[40px] h-[40px] bg-primary text-white hover:bg-[#FBE2C8]/40 hover:text-[#2E2D35]/80">
                {isPendingFacebookMessage ? (
                  <Loader variant="blue" />
                ) : (
                  <button
                    onClick={() => handleMessageSubmit()}
                    disabled={isPendingFacebookMessage}
                    className="cursor-pointer"
                  >
        ...
        {isAiAssistOpen && hasAiAssistAgent && isDesktopAiLayout ? (
          <div className="w-full h-[420px] border-t border-[#EEE7DD] lg:h-full lg:w-[360px] lg:min-w-[360px] lg:border-t-0">
            <AiAssist
              ...
              type={AI_SETTINGS_TYPES.FACEBOOK}
              chatId={chatID}
