...
const TIER_META: Record<SecurityTier, { label: string; desc: string; icon: any; color: string }> = {
  open: { label: 'Open', desc: 'No verification — bot calls directly', icon: LockOpen, color: 'text-gray-500 dark:text-mcm-ink-3' },
...
const CaptainActions = () => {
  ...
  return (
    <div className="flex h-full w-full flex-col gap-5 p-6">
      <div className="flex items-start gap-3 border-b border-gray-100 dark:border-mcm-line pb-5">
        <div className="flex size-11 shrink-0 items-center justify-center rounded-full bg-gray-900 text-white">
          <MessageSquare className="size-5" />
        </div>
        <div>
          <h2 className="text-lg font-bold text-[#2E2D35] dark:text-mcm-ink">Actions</h2>
      ...
      <div className="flex items-center gap-5 border-b border-[#EEE7DD] dark:border-mcm-line">
        <button
          type="button"
          onClick={() => setMainTab('my-actions')}
          className={`flex items-center gap-1.5 border-b-2 pb-2.5 text-sm font-medium ${mainTab === 'my-actions' ? 'border-primary text-[#2E2D35] dark:text-mcm-ink' : 'border-transparent text-[#9A948F] dark:text-mcm-ink-3'}`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-1.5 border-b-2 pb-2.5 text-sm font-medium ${mainTab === 'create-action' ? 'border-primary text-[#2E2D35] dark:text-mcm-ink' : 'border-transparent text-[#9A948F] dark:text-mcm-ink-3'}`}
        >
        ...
        <>
          ...
          {connections.length > 0 && (
            <div className="flex flex-col gap-2">
              <h3 className="text-sm font-bold text-[#2E2D35] dark:text-mcm-ink">Connected Apps</h3>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
              {connections.map((conn) => {
                const tk = toolkits.find((t) => t.slug === conn.toolkit_slug);
                return (
                  <div key={conn.id} className="flex flex-col gap-2 rounded-2xl border border-[rgba(225,200,165,0.9)] dark:border-mcm-line bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface backdrop-blur-[12px] p-4">
                    <div className="flex items-center gap-2">
                      {tk?.logo ? <img src={tk.logo} alt="" className="size-6 rounded" /> : <Plug className="size-5 text-[#9A948F] dark:text-mcm-ink-3" />}
                      <div className="truncate text-sm font-semibold text-[#2E2D35] dark:text-mcm-ink">{conn.toolkit_name}</div>
                    </div>
                    <p className="line-clamp-2 text-xs text-[#9A948F] dark:text-mcm-ink-3">{tk?.description || 'Connected app'}</p>
              ...
              {browseConnection && (
                <div className="flex flex-col gap-3 rounded-2xl border border-[rgba(225,200,165,0.9)] dark:border-mcm-line bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface backdrop-blur-[12px] p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-bold text-[#2E2D35] dark:text-mcm-ink">{browseConnection.toolkit_name} actions</div>
                      <p className="text-xs text-[#9A948F] dark:text-mcm-ink-3">Turn on the specific actions this assistant is allowed to call.</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-[#9A948F] dark:text-mcm-ink-3">Enable all</span>
                      <Switch checked={allBrowseActionsEnabled} onCheckedChange={(c) => setAllComposioActions(c === true)} />
                      <button type="button" onClick={() => setBrowseConnection(null)} className="ml-1 rounded-lg p-1.5 text-[#9A948F] dark:text-mcm-ink-3 hover:bg-[#FBE2C8]/45 dark:hover:bg-mcm-surface-3 hover:text-[#9A948F] dark:hover:text-mcm-ink-3">
                        ✕
                      </button>
                    </div>
                  </div>
                  ...
                  {isLoadingActions ? (
                    <div className="flex h-24 items-center justify-center text-sm text-[#9A948F] dark:text-mcm-ink-3">Loading actions...</div>
                  ) : (
                    <>
                      <div className="flex items-center gap-3">
                        <div className="relative flex-1">
                        ...
                        <label className="flex shrink-0 items-center gap-1.5 text-xs text-[#9A948F] dark:text-mcm-ink-3">
                          <Checkbox checked={allBrowseActionsEnabled} onCheckedChange={(c) => setAllComposioActions(c === true)} />
                          Select all
                        </label>
                      </div>
                      <div className="flex max-h-96 flex-col divide-y divide-gray-100 dark:divide-mcm-line overflow-y-auto rounded-xl border border-[#EEE7DD] dark:border-mcm-line">
                        {visibleBrowseActions.length === 0 ? (
                          <div className="px-4 py-6 text-center text-xs text-[#9A948F] dark:text-mcm-ink-3">No actions match your search.</div>
                        ) : (
                          visibleBrowseActions.map((a) => (
                            <div key={a.slug} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-[#FBE2C8]/45 dark:hover:bg-mcm-surface-3">
                              <div className="flex min-w-0 flex-1 items-center gap-3">
                                ...
                                <div className="min-w-0">
                                  <div className="flex items-center gap-1.5">
                                    <span className="truncate text-sm font-semibold text-[#2E2D35] dark:text-mcm-ink">{a.name}</span>
                                    <span className="shrink-0 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary">Composio</span>
                                  </div>
                                  <div className="truncate text-xs text-[#9A948F] dark:text-mcm-ink-3">{a.slug}</div>
                                </div>
                              </div>
                                ...
                                {a.operation_type === 'read' ? (
                                  <div className="relative">
                                    <button
                                      ...
                                      className={`flex items-center gap-1 rounded-lg border border-[#EEE7DD] dark:border-mcm-line bg-[#FBE2C8]/45 dark:bg-mcm-surface-3 px-2 py-1.5 text-xs font-medium hover:bg-[#FBE2C8]/40 dark:hover:bg-mcm-surface-3 ${a.security_tier ? TIER_META[a.security_tier].color : 'text-[#9A948F] dark:text-mcm-ink-3'}`}
                                    >
                                      {a.security_tier ? (
                                        <>
                                    ...
                                    {browseTierPopoverSlug === a.slug && (
                                      <>
                                        ...
                                        <div className="absolute right-0 top-full z-50 mt-1 w-64 rounded-xl border border-[rgba(225,200,165,0.9)] dark:border-mcm-line bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface backdrop-blur-[12px] shadow-xl">
                                          <div className="border-b border-gray-100 px-3 py-2 text-xs font-semibold text-[#2E2D35] dark:text-mcm-ink">Security level</div>
                                          <div className="py-1">
                                            {(['open', 'standard', 'secure'] as const).map((level) => {
                                              ...
                                              return (
                                                <button
                                                  ...
                                                  className={`flex w-full items-start gap-2.5 px-3 py-2.5 text-left hover:bg-[#FBE2C8]/45 dark:hover:bg-mcm-surface-3 ${active ? 'bg-[#FBE2C8]/45 dark:bg-mcm-surface-3' : ''}`}
                                                >
                                                  <Icon className={`mt-0.5 size-4 shrink-0 ${active ? meta.color : 'text-[#9A948F] dark:text-mcm-ink-3'}`} />
                                                  <span className="min-w-0 flex-1">
                                                    <span className={`block text-xs font-medium ${active ? meta.color : 'text-[#2E2D35] dark:text-mcm-ink'}`}>{meta.label}</span>
                                                    <span className="block text-xs text-[#9A948F] dark:text-mcm-ink-3">{meta.desc}</span>
                                                  </span>
                                  ...
                                  <span className="rounded-lg bg-[#FBE2C8]/45 dark:bg-mcm-surface-3 px-2 py-1.5 text-xs text-[#9A948F] dark:text-mcm-ink-3" title="Write actions are staff-only — never exposed to customers">
                                    Staff only
                                  </span>
                                )}
          ...
          {isLoading ? null : connections.length === 0 && filteredTools.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-[#EEE7DD] dark:border-mcm-line px-5 py-16 text-center">
              <Zap className="size-8 text-gray-300" />
              <div className="text-sm font-semibold text-[#2E2D35] dark:text-mcm-ink">No connected actions yet</div>
              <p className="max-w-xs text-xs text-[#9A948F] dark:text-mcm-ink-3">Connect an app or add a custom tool from the Create action tab.</p>
            ...
            <div className="flex flex-col gap-2">
              <h3 className="text-sm font-bold text-[#2E2D35] dark:text-mcm-ink">Custom Actions</h3>
              {filteredTools.map((t) => (
                <div key={t.id} className="flex items-center justify-between gap-4 rounded-2xl border border-[rgba(225,200,165,0.9)] dark:border-mcm-line bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface backdrop-blur-[12px] px-5 py-4">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <div className="truncate text-sm font-semibold text-[#2E2D35] dark:text-mcm-ink">{t.title}</div>
                        {t.kind === 'composio' && (
                          <span className="shrink-0 rounded-full bg-primary/10 px-2 py-0.5 text-[10px] font-medium text-primary">Composio</span>
                        )}
                      </div>
                      <div className="truncate text-xs text-[#9A948F] dark:text-mcm-ink-3">
                    ...
                    {t.operation_type === 'read' ? (
                      <div className="relative">
                        <button
                          ...
                          className={`flex items-center gap-1 rounded-lg border border-[#EEE7DD] dark:border-mcm-line bg-[#FBE2C8]/45 dark:bg-mcm-surface-3 px-2 py-1.5 text-xs font-medium hover:bg-[#FBE2C8]/40 dark:hover:bg-mcm-surface-3 ${t.security_tier ? TIER_META[t.security_tier].color : 'text-[#9A948F] dark:text-mcm-ink-3'}`}
                        >
                          {t.security_tier ? (
                            <>
      ...
      {mainTab === 'create-action' && (
        <>
          ...
          <h3 className="text-sm font-bold text-[#2E2D35] dark:text-mcm-ink">Browse apps to connect</h3>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
            <button type="button" onClick={openCreate} disabled={!selectedId} className="flex flex-col items-start gap-2 rounded-2xl border border-dashed border-[#EEE7DD] dark:border-mcm-line bg-[#FBE2C8]/50 dark:bg-mcm-surface-3 p-4 text-left hover:border-primary hover:bg-primary/[0.02] disabled:opacity-50">
              <div className="flex size-9 items-center justify-center rounded-full bg-primary/10 text-primary">
                <Wrench className="size-4" />
              </div>
              <div className="text-sm font-semibold text-[#2E2D35] dark:text-mcm-ink">Custom action</div>
              <p className="text-xs text-[#9A948F] dark:text-mcm-ink-3">Call any API, with your own auth and request/response templates.</p>
            </button>

            {isLoadingToolkits ? (
              <div className="col-span-full flex h-16 items-center justify-center text-sm text-[#9A948F] dark:text-mcm-ink-3">Loading apps...</div>
            ) : (
              visibleToolkits.filter((tk) => !connectionFor(tk.slug)).map((tk) => (
                <div key={tk.slug} className="flex flex-col gap-2 rounded-2xl border border-[rgba(225,200,165,0.9)] dark:border-mcm-line bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface backdrop-blur-[12px] p-4">
                  <div className="flex items-center gap-2">
                    {tk.logo ? <img src={tk.logo} alt="" className="size-6 rounded" /> : <Plug className="size-5 text-[#9A948F] dark:text-mcm-ink-3" />}
                    <div className="truncate text-sm font-semibold text-[#2E2D35] dark:text-mcm-ink">{tk.name}</div>
                  </div>
                  <p className="line-clamp-2 text-xs text-[#9A948F] dark:text-mcm-ink-3">{tk.description}</p>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-2xl p-6">
          <DialogTitle className="mb-4 text-base font-bold text-[#2E2D35] dark:text-mcm-ink">{editingId ? 'Edit Action' : 'Add Action'}</DialogTitle>
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              />
              <p className="text-xs text-[#9A948F] dark:text-mcm-ink-3">Leave blank to pass the raw JSON response straight to the model.</p>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {form.param_schema.map((p, i) => (
                <div key={i} className="flex items-center gap-2 rounded-lg border border-[#EEE7DD] dark:border-mcm-line p-2">
                  ...
                  <label className="flex items-center gap-1 text-xs text-[#9A948F] dark:text-mcm-ink-3">
            ...
            <div className="flex flex-col gap-2 rounded-xl border border-[#EEE7DD] dark:border-mcm-line p-3">
            ...
            <div className="flex flex-col gap-2 rounded-xl border border-[#EEE7DD] dark:border-mcm-line p-3">
              ...
              <div className="flex gap-3">
                {(['full', 'limited'] as const).map((opt) => (
                  <button key={opt} type="button" onClick={() => setForm((f) => ({ ...f, data_access: opt }))} className={`flex-1 rounded-xl border px-3 py-2 text-left text-xs ${form.data_access === opt ? 'border-primary bg-primary/5' : 'border-[#EEE7DD] dark:border-mcm-line'}`}>
                    <div className="font-medium text-[#2E2D35] dark:text-mcm-ink">{opt === 'full' ? 'Full response' : 'Limited fields'}</div>
                    <div className="text-[#9A948F] dark:text-mcm-ink-3">{opt === 'full' ? 'The model sees the entire API response' : 'Only the fields you list below reach the model'}</div>
                  </button>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-2 rounded-xl border border-[#EEE7DD] dark:border-mcm-line p-3">
              ...
              <p className="text-xs text-[#9A948F] dark:text-mcm-ink-3">Only read actions can ever be exposed to customers — write actions stay staff-only (Playground), no matter the security level.</p>
              <div className="flex gap-3">
                {(['read', 'write'] as const).map((opt) => (
                  <button key={opt} type="button" onClick={() => setForm((f) => ({ ...f, operation_type: opt }))} className={`flex-1 rounded-xl border px-3 py-2 text-left text-xs capitalize ${form.operation_type === opt ? 'border-primary bg-primary/5 text-primary' : 'border-[#EEE7DD] dark:border-mcm-line text-[#2E2D35] dark:text-mcm-ink'}`}>
                    {opt}
                  </button>
                ))}
              </div>
            ...
            <div className="flex items-center justify-between rounded-xl border border-[#EEE7DD] dark:border-mcm-line p-3">
              <div>
                <Label>Enabled</Label>
                <p className="text-xs text-[#9A948F] dark:text-mcm-ink-3">Off means the assistant can never call this action.</p>
              </div>
              <Switch checked={form.enabled} onCheckedChange={(c) => setForm((f) => ({ ...f, enabled: c === true }))} />
            </div>

            <div className="flex flex-col gap-2 rounded-xl border border-[#EEE7DD] dark:border-mcm-line bg-[#FBE2C8]/50 dark:bg-mcm-surface-3 p-3">
              ...
              {testResult && <pre className="max-h-40 overflow-auto whitespace-pre-wrap rounded-lg bg-[rgba(251,249,246,0.88)] dark:bg-mcm-surface backdrop-blur-[12px] p-2 text-xs text-[#2E2D35] dark:text-mcm-ink">{testResult}</pre>}
            </div>
          </div>
