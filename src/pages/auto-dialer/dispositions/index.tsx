...
const DispositionsList = () => {
          ...
          <span className="flex gap-2 items-center">
            <span
              className={`flex items-center justify-center rounded-full w-8 h-8 ${
                hasAccess
                  ? 'cursor-pointer bg-gray-100 dark:bg-mcm-surface-3 text-gray-900/80 dark:text-mcm-ink/80 hover:bg-primary hover:text-white'
                  : 'cursor-not-allowed bg-gray-100 dark:bg-mcm-surface-3 text-gray-400 dark:text-mcm-ink-3 opacity-50'
              }`}
              onClick={() => {
            ...
            >
              <Icon
                name="EditStrokIcon"
                className={`w-5 h-5 ${
                  campaign?.campaignStatus === 'PROCESSING' ? 'text-gray-400 dark:text-mcm-ink-3' : ''
                }`}
              />
            </span>
  ...
  return (
    <>
      <section className="dtable-page w-full bg-gray-200/15 dark:bg-mcm-surface-3/15 flex flex-col overflow-x-auto overflow-y-hidden  h-full">
